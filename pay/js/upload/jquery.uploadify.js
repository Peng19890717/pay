...
SWFUpload.prototype.initSWFUpload = function(b) {
	try {
		...
		this.movieName="SWFUpload_"+parseInt(10000000*Math.random());
		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				var settings = $.extend({
					...
					height          : 22,                 // The height of the browse button
		...
		onDialogClose :  function(filesSelected, filesQueued, queueLength) {
			...
			if ($.inArray('onDialogClose', settings.overrideEvents) < 0) {
				if (this.queueData.filesErrored > 0) {
					showOperationResult(this.queueData.errorMsg,"无法上传");
				}
			}
			...
			if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
				switch(errorCode) {
					...
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						...
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件 "' + file.name + '" 类型错误 (' + settings.fileTypeDesc + ').';
						break;
				}
			}
