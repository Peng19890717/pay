 ...
 *
 */
public class FY3PayService {
	private static final Log log = LogFactory.getLog(FY3PayService.class);
	//（钱通--富友）银行编码映射。
...
//		FY_B2C_BANK_CODE.put("SHRCB","0865012900");//上海农商
	...
	public void pay(HttpServletRequest request,PayProductOrder prdOrder,PayOrder payOrder){
		try{
			...
			String mchnt_cd = PayConstant.PAY_CONFIG.get("fy_mchnt_cd3");//商户编号
			...
			String back_notify_url = PayConstant.PAY_CONFIG.get("fy_back_notify_url3");//支付通知后台通知地址。
			String order_amt = payOrder.txamt.toString();//订单金额。
			String goods_name = "";//商品名称。
			String goods_display_url="";//商品展示网址。
			String rem="";//备注。
			String mchnt_key=PayConstant.PAY_CONFIG.get("fy_mchnt_key3");//商户编号
	...
	public void channelQuery(PayOrder payOrder) throws Exception{
		try{
			...
			String page_notify_url = PayConstant.PAY_CONFIG.get("fy_page_notify_url3");//支付通知地址。
			String back_notify_url = PayConstant.PAY_CONFIG.get("fy_back_notify_url3");//支付通知后台通知地址。
			String mchnt_key=PayConstant.PAY_CONFIG.get("fy_mchnt_key3");//秘钥。
			//MD5摘要数据。
	...
	public String  quickPay(HttpServletRequest request,PayRequest payRequest,PayProductOrder prdOrder,PayOrder payOrder)throws Exception{
		try {
			String key = PayConstant.PAY_CONFIG.get("fy_quickPay_key3"); //富友分配的32位商户秘钥key。
			...
			String mchntcd =PayConstant.PAY_CONFIG.get("fy_quickPay_mchntcd3"); //商户编码。
			...
			String backurl = PayConstant.PAY_CONFIG.get("fy_quickPay_backurl3"); //结果异步通知路径。
	...
	public void quickPaychannelQuery(PayOrder payOrder) throws Exception{
		Blog log = new Blog();
		try{
			String mchntcd =PayConstant.PAY_CONFIG.get("fy_quickPay_mchntcd3"); //商户号。
			String OrderId = payOrder.bankjrnno; //富友下单后返回的订单号。
			String key = PayConstant.PAY_CONFIG.get("fy_quickPay_key3"); 
