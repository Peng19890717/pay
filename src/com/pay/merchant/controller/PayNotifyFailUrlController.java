...
import com.pay.merchant.service.PayNotifyFailUrlService;
@Controller
public class PayNotifyFailUrlController {
    @RequestMapping("payNotifyFailUrl")
    public String getPayNotifyFailUrlList(HttpServletRequest request,HttpServletResponse response,
            @ModelAttribute("payNotifyFailUrl") PayNotifyFailUrl payNotifyFailUrl) {
        ...
        new Blog(this.getClass().getSimpleName()+".getPayNotifyFailUrlList","取得PayNotifyFailUrl列表",user.id,request);
        OutputStream os = null;
        try {
            if(flag == null)return "/jsp/pay/merchant/pay_notify_fail_url.jsp";
            ...
            os.write(new PayNotifyFailUrlService().getPayNotifyFailUrlList(payNotifyFailUrl,page,rows,
