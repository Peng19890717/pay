...
public class SmsLogController {
    ...
    public String getSmsLogList(HttpServletRequest request,HttpServletResponse response,
            @ModelAttribute("smsLog") SmsLog smsLog) {
        ...
        //new Blog(this.getClass().getSimpleName(),"取得SmsLog列表",user.id,request);
        OutputStream os = null;
        try {
            if(flag == null)return "/jsp/common/sms/sms_log.jsp";
            ...
            os.write(new SmsLogService().getSmsLogList(smsLog,page,rows,
    ...
    public String addSmsLog(HttpServletRequest request,HttpServletResponse response,
            @ModelAttribute("smsLog") SmsLog smsLog) {
        JWebUser user = (JWebUser)request.getSession().getAttribute("user");
        OutputStream os = null;
        Blog log = null;
        try {
            ...
            log = new Blog(this.getClass().getSimpleName(),"添加SmsLog",user.id,request);
            smsLog.createId = user.id;
            new SmsLogService().addSmsLog(smsLog);
    ...
    @RequestMapping("removeSmsLog")
    public String removeSmsLog(HttpServletRequest request,HttpServletResponse response) {
        JWebUser user = (JWebUser)request.getSession().getAttribute("user");
        OutputStream os = null;
        Blog log = null;
        try {
            ...
            log = new Blog(this.getClass().getSimpleName(),"删除SmsLog",user.id,request);
            new SmsLogService().removeSmsLog(request.getParameter("id"));
            os.write(JWebConstant.OK.getBytes());
        } catch (Exception e) {
            e.printStackTrace();
            try {os.write(e.getMessage().getBytes("utf-8"));} catch (Exception e1) {}
    ...
    @RequestMapping("updateSmsLog")
    public String updateSmsLog(HttpServletRequest request,HttpServletResponse response,
            @ModelAttribute("smsLog") SmsLog smsLog) {
        JWebUser user = (JWebUser)request.getSession().getAttribute("user");
        OutputStream os = null;
        Blog log = null;
        try {
            ...
            log = new Blog(this.getClass().getSimpleName(),"修改SmsLog",user.id,request);
            new SmsLogService().updateSmsLog(smsLog);
            os.write(JWebConstant.OK.getBytes());
        } catch (Exception e) {
            e.printStackTrace();
            try {os.write(e.getMessage().getBytes("utf-8"));} catch (Exception e1) {}
