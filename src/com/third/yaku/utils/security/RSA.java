...
public class RSA
{
	...
	public static String unsign(String content, String publicKey, String input_charset)
	{
		try
		{
			PKCS8EncodedKeySpec priPKCS8 = new PKCS8EncodedKeySpec(Base64.decode(publicKey));
			KeyFactory keyf = KeyFactory.getInstance("RSA");
			PublicKey pubey = keyf.generatePublic(priPKCS8);
			java.security.Signature signature = java.security.Signature.getInstance(SIGN_ALGORITHMS);
			signature.initVerify(pubey);
			signature.update(content.getBytes(input_charset));
