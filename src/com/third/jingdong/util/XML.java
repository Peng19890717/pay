package com.third.jingdong.util;
...
public class XML {
	...
	private static Map<String, String> parseElements(Map<String, String> map, Element element, String name) throws Exception {
		...
		{
			map.put(name==null||name.trim().length()==0 ? element.getName().toLowerCase() : name, element.getText());
		}
		else
		{
			for(Element _element : elementList)
			{
				map = parseElements(map, _element, name==null||name.trim().length()==0 ? null : (name + "." +_element.getName().toLowerCase()));
			}
		}
